...
class LinearRegression:
    ...
    """
    Linear Regression with gradient descent, normalization, and optional L2 regularization.
    ...
    normalize : {'zscore', 'minmax', None}, default='zscore'
        Normalization strategy.
    tol : float, default=1e-6
        Tolerance for early stopping.
    patience : int, default=10
        Number of iterations to wait for improvement before stopping.
    reg_type : {'l2', None}, default=None
        Regularization type.
    reg_strength : float, default=0.01
        Regularization strength (used only if reg_type='l2').
    
    Attributes
    ----------
    w : np.ndarray
        Learned weights (including bias as w[0]).
    _is_fitted : bool
        ...
        for i in range(self.n_iterations):
            ...
            if loss < best_loss - self.tol:
                ...
                if patience_counter >= self.patience:
                    print(f"Early stopping at iteration {i}, MSE: {loss:.6f}")
                    break

            error = y_pred - y
            dw = (2.0 / n_samples) * X_with_bias.T @ error
            ...
            if i % 1000 == 0:
                print(f"Iteration {i}, MSE: {loss:.6f}")
